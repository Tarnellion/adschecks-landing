...
|---|---|---|
| `PUBLIC_GA_ID` | `G-XGQWL529CS` — **уже задан** в `.env.production`, в дашборде ничего делать не нужно | — |

Идентификатор потока GA4 не является секретом: он и так отдаётся в HTML каждой страницы.
Поэтому он лежит в `.env.production` в репозитории, и аналитика работает без настройки
в дашборде. Переменная окружения в Cloudflare, если её всё же задать, **перебивает**
значение из файла — проверено сборкой.
...
не попадает ни баннер согласия, ни код gtag — вообще ничего. `.env.production` читается
только при `astro build`, поэтому `npm run dev` остаётся чистым.

Отдельно от этого превью-деплои Cloudflare отсекаются в рантайме: скрипт согласия
выходит сразу, если хост оканчивается на `.pages.dev`. Иначе трафик с превью-сборок
попадал бы в тот же GA-ресурс, что и продакшен, и портил статистику.
